...
exports.getAllPlayer = async (req, res) => {
  try {
    ...
    const players = await Player.findAll(); //+ models 없이 바로 Player로 접근할 수 있다. 구조분해할당 이용
    console.log(players);
    res.send(players);
  } catch (err) {
    console.log("err", err);
...
exports.getPlayer = async (req, res) => {
  try {
    ...
    const player = await Player.findOne({
      ...
      //  include: 두 테이블을 합쳐서 보여줌 (기본 inner join)
    });
    res.json(player);
  } catch (err) {
    console.log("err", err);
...
exports.postPlayer = async (req, res) => {
  try {
    ...
    const newPlayer = await Player.create({
      // 보내려는 변수 name, DB 컬럼명도 name이다
      // name: name 형태가 아닌 name만 써도 ok
      name,
      age,
      teamid,
    });
...
exports.getTeams = async (req, res) => {
  try {
    ...
    //   sort = name_asc >> 전체 조회 및 이름 순 팀 정렬
    ...
    if (sort === "name_asc") {
      ...
      // sort에 name_asc 외의 문자열이 들어오거나 sort나 search가 전달되지 않았을 때
...
exports.getTeamPlayers = async (req, res) => {
  try {
    // 1. 팀 정보는 안봐도 되고, 특정 팀의 선수 정보만 확인하기 위해서는
    // "Player 모델"에서 teamid 기준으로 조회
    const { teamId } = req.params;
