const crypto = require("crypto"); // nodejs 내장 모듈, 설치 x
/* 
...
console.log("2st hashing", createHashPW("1234"));
console.log("3st hashing", createHashPW("1234")); // 전부 다 똑같은 값
...
function saltAndHashPW(pw) {
  ...
  const hash = crypto
    .pbkdf2Sync(pw, salt, iteration, keylen, algorithm) // buffer
...
function checkPW(inputPW, savedSalt, savedHash) {
  const iteration = 100; // salt 제외 아래 전부 saltAndHashPW와 같아야 함
  const keylen = 64;
  const algorithm = "sha512";
...
// 로그인 >> 비밀번호가 db의 hash 값과 일치하는 지 확인
...
function decrypt(encryptedText) {
  ...
  // 2. 실제 데이터 복호화
  // base64로 인코딩 된 문자열이 utf8 형태로 복호화된다
