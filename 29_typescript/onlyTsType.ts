...
interface 야구부 extends Student {
  ...
  height: number;
  weight: number;
  readonly backNumber?: number; // 필수값이 아닐 때: ? 처리
  [grade: number]: Score; // key가 숫자형인 게 여러 개 올 수 있다.
}
...
interface Toy {
  name: string;
  start(): void; //+ 함수의 return 값이 없을 때는 void
}
interface Car {
  name: string;
  color: string;
...
// daniel.gender = "여성"; // 불가
// daniel.gender = "female"; // 불가
//+ daniel.gender = "Male"; // 가능
console.log(daniel);
