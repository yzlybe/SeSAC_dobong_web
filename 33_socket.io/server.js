...
io.on("connection", (socket) => {
  ...
  socket.on("join", (chatRoom) => {
    ...
    // console.log("방 만들어진 후,", socket.rooms);
    socket.room = chatRoom; //+ socket.room은 기본 제공은 아니고 room이라는 정보를 직접 넣어주는 것

    // 3. 내가 포함된 방 client에게 입장 메세지 전달
  ...
  socket.on("message", (message) => {
    ...
    io.to(socket.room).emit("message_toAll", message, socket.id); //+ chatroom은 join에서만 쓸 수 있는 정보.
    // chatroom의 정보를 socket.room에 저장해뒀기 떄문에 쓸 수 있다.
  });
});
