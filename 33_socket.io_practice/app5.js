...
io.on("connection", (socket) => {
  // [닉네임 사용2]: 닉네임 중복 체크 후 중복이 아닐 때만 nickInfo에 정보 추가
  socket.on("checkNick", (nickname) => {
    // console.log(nickInfo);
    // console.log(Object.values(nickInfo)); //+ object의 value값만을 배열로 반환해준다
    ...
    if (Object.values(nickInfo).indexOf(nickname) > -1) {
      // 중복된 데이터
      // (1) 입장 실패, 닉네임 중복 시 에러메세지를 클라이언트에 보냄
      ...
      // (3) 입장 성공, ~~~님이 입장하셨습니다. 알림메세지 전달 (나 제외)
